...
class RAGPipeline:
    ...
    def ingest_and_describe_csv(self, csv_path, force_refresh=False):
        ...
        if not check or force_refresh:
            ...
            snippet_str = self.db.conn.execute(
    ...
    def retrieve_relevant_table(self, user_query):
        ...
        with open(os.path.join(config.CSV_METADATA_DIR, f"{best_name}_info.json"), 'r') as f:
            return json.load(f)
    ...
    def get_query_intent(user_query):
        ...
        if any(w in q for w in config.CSV_AGGREGATE_WORDS):
            return "AGGREGATE"
        if any(w in q for w in config.SCAN_WORDS):
            return "SCAN"
        return "LOOKUP"
    ...
    def generate_and_execute_sql(self, user_query, target_table_info):
        ...
        if intent == "LOOKUP":
            ...
            intent_rule = (
                "RULE: Use SELECT * with LIMIT for scan queries. No WHERE clause.\n"
                f'Example: SELECT * FROM "{table_name}" LIMIT 10'
            )
            prefill = f'SELECT * FROM "{table_name}"'
        else:
            intent_rule = (
        ...
        try:
            result_df = self.db.conn.execute(clean_sql).df()
